...
return access(this,function(elem,type,value){var doc;return jQuery.isWindow(elem)?elem.document.documentElement["client"+name]:9===elem.nodeType?(doc=elem.documentElement,Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])):void 0===value?jQuery.css(elem,type,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:void 0,chainable,null)}})}),jQuery.fn.extend({bind:function(types,data,fn){return this.on(types,null,data,fn)},unbind:function(types,fn){return this.off(types,null,fn)},delegate:function(selector,types,data,fn){return this.on(types,selector,data,fn)},undelegate:function(selector,types,fn){return 1===arguments.length?this.off(selector,"**"):this.off(types,selector||"**",fn)},size:function(){return this.length}}),jQuery.fn.andSelf=jQuery.fn.addBack,"function"==typeof define&&define.amd&&define("jquery",[],function(){return jQuery});var _jQuery=window.jQuery,_$=window.$;return jQuery.noConflict=function(deep){return window.$===jQuery&&(window.$=_$),deep&&window.jQuery===jQuery&&(window.jQuery=_jQuery),jQuery},noGlobal||(window.jQuery=window.$=jQuery),jQuery}),function(global,$,undefined){"use strict";var Base=global.Base=function(){this.init.apply(this,arguments)};return $.extend(Base.prototype,{extend:$.extend,each:$.each,map:$.map,deferred:$.Deferred,init:function(){}}),Base.extend=function(protoProps,staticProps){var child,parent=this;return child=protoProps&&protoProps.hasOwnProperty("constructor")?protoProps.constructor:function(){return parent.apply(this,arguments)},$.extend(child,parent,staticProps),child.prototype=Object.create(parent.prototype),$.extend(child.prototype,protoProps),child.prototype.constructor=child,child.__super__=parent.prototype,child.prototype.__super__=child.__super__,child},Base}(window,jQuery),function(global,$,Base,undefined){"use strict";var Event=global.Event=Base.extend({init:function(){this.handlers={}},on:function(name,fn){if("string"!=typeof name)throw new TypeError('"name" must be a string type');if("function"!=typeof fn)throw new TypeError('"fn" must be a function');this.handlers[name]=this.handlers[name]||[],this.handlers[name].push(fn)},trigger:function(name){var i,len;if(this.handlers[name])for(i=0,len=this.handlers[name].length;len>i;i++)this.handlers[name][i].apply(this,arguments)}});return Event}(window,jQuery,Base),function(global,Base,undefined){"use strict";var Model=global.Model=Base.extend({init:function(data){var self=this;if(this._attributes={},!data||"object"!=typeof data)throw new TypeError('"data" must be a object type');if(!this.storage)throw new TypeError('"storage" must be defined');if(!this.schema)throw new TypeError('"schema" must be defined');this.each(this.schema,function(key){self.setAttribute(key,data[key])})},setAttribute:function(key,value){if(!this.schema[key])throw new TypeError('property "'+key+'" is unacceptable');if(this.schema[key].required&&"undefined"==typeof value)throw new TypeError('property "'+key+'" is required');if("undefined"==typeof value){if(!("defaultValue"in this.schema[key]))return this;value=this.schema[key].defaultValue}return this.schema[key].type?"Date"===this.schema[key].type?this._attributes[key]=new global[this.schema[key].type](value):this._attributes[key]=global[this.schema[key].type](value):this._attributes[key]=value,this},getAttribute:function(key){if(!this.schema[key])throw new TypeError('property "'+key+'" is unacceptable');return this._attributes[key]},save:function(){var self=this;return this._attributes.id?this.storage.update(this._attributes.id,this._attributes).pipe(function(item){return self.each(self.schema,function(key){self.setAttribute(key,item[key])}),self}):this.storage.create(this._attributes).pipe(function(item){return new self.constructor(item)})},remove:function(id){return this.storage.remove(id)}},{create:function(value){var self=this,item=new self.prototype.constructor(value);return item.save()},get:function(id){var self=this;return this.prototype.storage.fetchOne(id).pipe(function(item){return new self.prototype.constructor(item)})},find:function(){var self=this;return self.prototype.storage.fetch().pipe(function(list){return self.prototype.each(list,function(i,item){list[i]=new self.prototype.constructor(item)}),list})}});return Model}(window,Base),function(global,$,Base,undefined){"use strict";var View=global.View=Base.extend({$:$,init:function(){this._inserted=!1,this.data={},this.el=this.$(this.template)},setData:function(data){return this.extend(this.data,data),this},render:function(parent){return!parent||this._inserted?this:(this.$(parent).append(this.el),this._inserted=!0,this)}},{template:function(templateId){return this.prototype.$("#"+templateId).html()}});return View}(window,jQuery,Base),function(global,$,Base,Event,undefined){"use strict";var Controller=global.Controller=Base.extend({init:function(){var self=this;self.event=new Event,"object"==typeof self.events&&self.each(self.events,function(name,handler){self.event.on(name,function(){handler.apply(self,arguments)})})}});return Controller}(window,jQuery,Base,Event),function(global,Base,undefined){"use strict";var Storage=global.Storage=Base.extend({storage:global.localStorage,init:function(name){if(!name||"string"!=typeof name)throw new TypeError('"name" must be a string type');this.name=name},_fetchData:function(){return JSON.parse(this.storage[this.name+"Collection"]||"[]")},_saveData:function(data){this.storage[this.name+"Collection"]=JSON.stringify(data)},_getIndexById:function(id,data){var i,len;for(i=0,len=data.length;len>i;i++)if(data[i].id=id)return i;return-1},create:function(value){var dfd=this.deferred(),data=this._fetchData(),increment=(100*Date.now()+Math.round(100*Math.random()),this.storage[this.name+"Autoincrement"]);return increment=increment?parseInt(increment):0,value.id=++increment,this.storage[this.name+"Autoincrement"]=String(increment),value.created=value.updated=new Date,data.push(value),this._saveData(data),dfd.resolve(value),dfd.promise()},update:function(id,value){var dfd=this.deferred(),data=this._fetchData(),index=this._getIndexById(id,data);return-1===index?(dfd.reject("Not found"),dfd.promise()):(this.extend(data[index],value),value.updated=new Date,this._saveData(data),dfd.resolve(data[index]),dfd.promise())},fetchOne:function(id){var dfd=this.deferred(),data=this._fetchData(),index=this._getIndexById(id,data);return-1===index?dfd.reject("Not found"):dfd.resolve(data[index]),dfd.promise()},fetch:function(){var dfd=this.deferred();return dfd.resolve(this._fetchData()),dfd.promise()},remove:function(id){var dfd=this.deferred(),data=this._fetchData(),index=this._getIndexById(id,data);return data.splice(index,1),this._saveData(data),dfd.resolve(),dfd.promise()},clear:function(){var dfd=this.deferred();return this.storage.removeItem(this.name+"Autoincrement"),this._saveData([]),dfd.resolve(),dfd.promise()}});return Storage}(window,Base),function(global,Model,Storage,undefined){"use strict";var MessageModel=global.MessageModel=Model.extend({storage:new Storage("message"),schema:{id:{type:"Number",required:!1},parentId:{type:"Number",required:!1,defaultValue:0},text:{type:"String",required:!0},avatar:{type:"String",required:!1,defaultValue:"/img/default-avatar.png"},created:{type:"Date",required:!1},updated:{type:"Date",required:!1}}});return MessageModel}(window,Model,Storage),function(global,View,undefined){"use strict";var ChatFormView=global.ChatFormView=View.extend({template:View.template("chatForm"),init:function(event){var self=this,submitHandler=function(){var value=self.valueEl.val();value&&self.event.trigger("submit",{parentId:self.parentId||0,text:value})};this._isOpened=!1,this.__super__.init.call(this),this.event=event,this.hide(),this.el.click(function(e){e.stopPropagation()}),this.$("body").click(function(){self.close(!0)}),this.valueEl=this.el.find("[data-value]"),this.submitEl=this.el.find("[data-submit]"),this.submitEl.click(submitHandler),this.valueEl.keypress(function(e){13!==e.which||e.shiftKey||submitHandler()})},hide:function(){this.el.hide()},show:function(){var self=this;this.el.show(),setTimeout(function(){self.valueEl.focus()})},open:function(parentId){var self=this;this.parentId=parentId,this.el.slideDown(function(){self._isOpened=!0,self.valueEl.focus()})},close:function(noForced){var self=this;!this._isOpened&&noForced||(this._isOpened=!1,this.el.slideUp(function(){self.valueEl.val(""),self.parentId=0}))}});return ChatFormView}(window,View),function(global,View,undefined){"use strict";var ChatListItemView=global.ChatListItemView=View.extend({template:View.template("chatListItem"),init:function(event){var self=this;this.__super__.init.call(this),this.listEl=this.el.find("[data-chat-list]"),this.textEl=this.el.find("[data-text]"),this.avatarEl=this.el.find("[data-avatar]"),this.el.find(".reply").click(function(){event.trigger("reply",self.data.item.getAttribute("id"))})},render:function(parent){return this.__super__.render.call(this,parent),this.textEl.text(this.data.item.getAttribute("text")),this.avatarEl.attr("src",this.data.item.getAttribute("avatar")),this}});return ChatListItemView}(window,View),function(global,View,ChatListItemView,undefined){"use strict";var ChatListView=global.ChatListView=View.extend({template:View.template("chatList"),init:function(event){this.__super__.init.call(this),this.itemsIndex={},this.event=event},render:function(parent){var lastEl,self=this;return self.__super__.render.call(self,parent),self.data.items&&self.data.items.length>0&&(self.each(self.data.items,function(key,item){var itemView,id=item.getAttribute("id");self.itemsIndex[id]||(itemView=new ChatListItemView(self.event),itemView.setData({item:item}),self.itemsIndex[id]=itemView,lastEl=itemView.el)}),self.each(self.itemsIndex,function(id,itemView){var parentEl,parentId=itemView.data.item.getAttribute("parentId");parentEl=0===parentId?self.el:self.itemsIndex[parentId].listEl,itemView.render(parentEl)}),lastEl&&self.$("html, body").animate({scrollTop:lastEl.offset().top-parseInt($(".main").css("padding-top"))},400)),self}});return ChatListView}(window,View,ChatListItemView),function(global,Controller,MessageModel,ChatListView,ChatFormView,undefined){"use strict";var ChatController=global.ChatController=Controller.extend({init:function(){var self=this;self.__super__.init.call(this),self.listView=new ChatListView(self.event),self.formView=new ChatFormView(self.event),self.formView.render("#chatView"),self.load()},events:{reply:function(event,id){this.formView.open(id)},submit:function(event,data){var self=this;MessageModel.create(data).done(function(){self.load(),self.formView.close()}).fail(function(err){console.log(err)})}},load:function(){var self=this;MessageModel.find().done(function(items){self.listView.setData({items:items}).render("#chatView"),0===items.length&&self.formView.show()}).fail(function(err){console.log("err:",err)})}});return ChatController}(window,Controller,MessageModel,ChatListView,ChatFormView),function(ChatController,Storage,MessageModel){"use strict";var tmpStorage=new Storage("tmp");tmpStorage.fetchOne("mockupFirstLoaded").fail(function(){var mockupText="Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";MessageModel.create({text:mockupText}).done(function(message){MessageModel.create({parentId:message.getAttribute("id"),text:mockupText})}),MessageModel.create({text:mockupText}),tmpStorage.create({id:"mockupFirstLoaded"})}),new ChatController}(ChatController,Storage,MessageModel);